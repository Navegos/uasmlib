...
{
public: uX_gen_cpu_gpu_inline uXCompileTimeAssert() {};
};

template <> class uXCompileTimeAssert<false>
{	// generate compile-time error if false
private: uX_gen_cpu_gpu_inline uXCompileTimeAssert() {};
};
#endif //__cplusplus
...
#define uX_assert_msg(Expr, Msg)																				\
	{																											\
		...
		((void)((!!(Expr)) || (!_ignore &&																		\
		(uX::detail::GetAssertHandler()(uX_Stringer(Msg), uX_Stringer(Expr), __FILE__, __LINE__, _ignore), false))));	\
		uX_code_analysis_assume(Expr);																			\
	}

#define uX_ensure_msg(Expr,Msg)						((!!(Expr)) || (uX::detail::GetAssertHandler()(uX_Stringer(Msg), uX_Stringer(Expr), __FILE__, __LINE__), false))
...
#define uX_ensure_return_nullptr_msg(Expr,Msg)		if(!(Expr)) { uX_assert_msg(Expr, Msg); return nullptr; }
...
#define uX_ensure_return_nullptr(Expr)				if(!(Expr)) { uX_assert(Expr); return nullptr; }
...
#define uX_ensure_return_nullptr_msg(Expr,Msg)		if(!(Expr)) { return nullptr; }
...
#define uX_ensure_return_nullptr(Expr)				if(!(Expr)) { return nullptr; }
#define uX_ensure_return_val(Expr,Val)				if(!(Expr)) { return Val; }
#endif
#else
#warning "WARNING: uX_ASSERT_H is only supported in C++ compilations!"
#endif /* __cplusplus */
