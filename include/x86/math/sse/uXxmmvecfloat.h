...
namespace_uX
enum class vecelement : uint8_t {
    _x = 0, _y = 1, _z = 2, _w = 3
};

enum class matrixelement : uint8_t {
...
public:
     ...
     * @param Inxmm value to copy from.
     * /
    uX_constexpr vecfloat(const vecfloat3 Inxmm) uX_noexcept : m128_xmm(_uX_MM_SHUFFLER_IM_PS(Inxmm, _uX_mm_unpackhi_ps(Inxmm, m128_xmm), 0, 1, 0, 3)) {}
     ...
     * @param Inxmm value to copy from.
     * /
    uX_constexpr vecfloat(const vecfloat2 Inxmm) uX_noexcept : m128_xmm(_uX_MM_SHUFFLER_IM_PS(Inxmm, m128_xmm, 0, 1, 2, 3)) {}
