                           ...
                           const __m128d Inxmmd_4, const __m128d Inxmmd_5, const __m128d Inxmmd_6, const __m128d Inxmmd_7) uX_noexcept
{
    ...
    parallel_for(blocked_range<uint32_t>(0, m128_xmmd_ptr_lenght, 1),
...
vecdouble4x3::vecdouble4x3(const __m128d* Inpxmmd, uint32_t idxbegin, uint32_t idxend) uX_noexcept
{
    static affinity_partitioner affp;
    static task_group_context tgctx;
                 ...
                 [=](const blocked_range<uint32_t>& r, task_group_context const&) {
                     for (auto i = r.begin(); i < r.end(); ++i)
                     {
                         switch (i)
                         {
                             ...
                             case 7: m128_xmmd_7 = _uX_mm_set_pd(0.0, Indouble_W2); break;
                         }
                     }
                 }, affp, tgctx
    );
...
vecdouble4x3::vecdouble4x3(const double* Inpdouble, uint32_t idxbegin, uint32_t idxend) uX_noexcept
{
    static affinity_partitioner affp;
    static task_group_context tgctx;
    parallel_for(blocked_range<uint32_t>(idxbegin, idxend, 1),
                 [=](const blocked_range<uint32_t>& r, task_group_context const&) {
                     for (auto i = r.begin(); i < r.end(); ++i)
                     {
                         switch (i)
                         {
                             ...
                             case 6: m128_xmmd_6 = _uX_mm_set_pd(Inpdouble[10], Inpdouble[9]); break;
                             case 7: m128_xmmd_7 = _uX_mm_set_pd(0.0, Inpdouble[11]); break;
                         }
                     }
                 }, affp, tgctx
    );
